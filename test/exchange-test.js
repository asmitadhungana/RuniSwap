...
describe("Exchange", () => {
  ...
  it("is deployed", async () => {
    ...
    expect(await exchange.name()).to.equal("Runiswap-V1");
    expect(await exchange.symbol()).to.equal("RUNI-V1");
    expect(await exchange.totalSupply()).to.equal(toWei(0));
  });

  describe("addLiquidity", async () => {
    describe("when reserve is empty", async () => {
      ...
      it("mints LP tokens", async () => {
        ...
        expect(await exchange.balanceOf(owner.address)).to.eq(toWei(100)); //100 LP tokens (RUNI)
    ...
    describe("when reserve pre-exists", async () => {
      beforeEach(async () => {
        await token.approve(exchange.address, toWei(300));
        await exchange.addLiquidity(toWei(200), { value: toWei(100) });
      });

      it ("preserves the exchange rate", async () => {
      ...
      it("mints LP tokens", async () => {
        ...
        expect(await exchange.balanceOf(owner.address)).to.eq(toWei(150)); //100 LP tokens from adding liq into empty reserve [beforeEach] + 50 from here
  ...
  describe("removeLiquidity", async () => {
    ...
    it ("removes some liquidity", async () => {
      ...
      ).to.equal("24.999490448"); // 25 - gas fees [1%]
